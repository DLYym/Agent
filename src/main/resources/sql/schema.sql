CREATE TABLE IF NOT EXISTS logical_relation (
    ...
    relation_category VARCHAR(20) DEFAULT 'FOREIGN_KEY' COMMENT '关系类别: FOREIGN_KEY-外键关联, DICT_MAPPING-字典映射',
    description VARCHAR(500) DEFAULT NULL COMMENT '业务描述: 存入Prompt中帮助LLM理解 (例如: 订单表通过buyer_uid关联用户表id) | 字典映射时存放字典过滤条件(如dict_type=loan_purpose)',
